...
public class SpikesBlock extends Block implements SimpleWaterloggedBlock {
    ...
    public SpikesBlock() {
        super(Block.Properties.of(Material.METAL).noOcclusion().strength(0.35F, 0.35F).sound(SoundType.METAL).randomTicks());
    ...
    public void entityInside(BlockState state, Level world, BlockPos pos, Entity entity) {
        ...
        if (entity.fallDistance >= 5F) {
            entity.hurt(damageSource, 2F + entity.fallDistance * 2f);
            return;
        }
        else {
            final float damageTodo = (float)entity.getDeltaMovement().dot(new Vec3(1, 1, 1)) / 1.25F;
