...
public class FanTileEntity extends BlockEntity  {
    ...
    public static void tick(Level level, BlockPos worldPosition, BlockState blockState, FanTileEntity blockEntity) {
        ...
        for (final Entity entity : list) {
            ...
            //velocity += ((level.getBestNeighborSignal(worldPosition) >= 10) ? (0.03D * level.getBestNeighborSignal(worldPosition)) : 0D);
    ...
    public static void spawnParticles(final Level world, final BlockState blockState, final BlockPos pos) {
        ...
        final Direction facing = blockState.getValue(FanBlock.FACING);
        final double velocity = 0.2F + world.random.nextFloat() * 0.4f;//((world.getBestNeighborSignal(pos) >= 10) ? (0.03F * world.getBestNeighborSignal(pos)) : 0.4f);
